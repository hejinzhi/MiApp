...
export class EquipService {
    ...
    getMachineCheckList(id: string) {
        return Observable.forkJoin(this.getMachineLocationByName(id), Observable.fromPromise(this.myHttp.get(EquipConfig.getMachineCheckList.replace('{machineNO}', id))))
            .map((res => {
                let macAll = res[0].json();
                let mac = macAll && macAll.length > 0 ? macAll[0] : '';
                let li = res[1].json();
                if (li) {
                    li.LOCATION = mac ? mac.LOCATION1 + '_' + mac.LOCATION2 + '_' + mac.LOCATION3 + '_' + mac.LOCATION4 : '';
                }
                return li;
            }))
    }
    ...
    getOwnUndoneReport(waiting: boolean = false, cb?: Function) {
        this.bossService.getOwnUndoneReport(waiting, EquipConfig.type, cb);
    }

    getMachineLocationByName(name: string) {
        return Observable.fromPromise(this.getMachineList('', '', '', name));
    }
    ...
    updateQquipReport(data: any) {
        ...
        if (request.length > 0) {
            return Observable.forkJoin(...request).concatMap((l) => {
                console.log(l);
                console.log(send);
    ...
    ObserveOwnLinesCount() {
        return this.$store.select('linesEquipReducer').map((lines: BossReportLineState[]) => lines.length);
    }

    ObserveAdminLinesDealCount() {
        return this.$store.select('linesAllEquipReducer').map((lines: BossReportLineState[]) => lines.filter((l) => l.PROBLEM_STATUS === 'New').length);
    }
...
export class EquipReportModule implements BossReportState {
    ...
    constructor(data: any) {
        ...
        this.Header.INSPECTOR_NAME = data.INSPECTOR_NAME;
