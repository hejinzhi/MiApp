...
@IonicPage()
@Component({
  selector: 'sg-sale-day',
  templateUrl: 'sale-day.component.html',
})
export class SaleDayComponent {
  ...
  initOption(info:TableModel) {
    let option1 = JSON.parse(OptionsConfig.saleAchievement.option1);
    this.tableInfo.data.slice(1).forEach((list,idx:number) => {
      list.slice(1).forEach((str,subIdx:number) => {
   ...
   */
  async initInfo() {
    let res:any = await this.chartService.getSaleDayChartInfo().catch((e) => {
